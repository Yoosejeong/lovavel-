...
import com.example.locavel.apiPayload.exception.handler.TempHandler;
...
public class S3Uploader {
    ...
    public List<String> saveFiles(List<MultipartFile> multipartFiles) {
        ...
        for (MultipartFile multipartFile : multipartFiles) {
            if (isDuplicate(multipartFile)) {
                //throw new TempHandler("file", ErrorStatus.DUPLICATE_IMAGE);
                throw new TempHandler(ErrorStatus.NOT_IMAGE_EXTENSION);
            }
    ...
    public void deleteFile(String fileUrl) {
        ...
        if (!fileBucket.equals(bucket)) {
            //throw new TempHandler("fileUrl", ErrorStatus.NO_IMAGE_EXIST);
            throw new TempHandler(ErrorStatus.NOT_IMAGE_EXTENSION);
        }
        ...
        try {
            ...
            throw new TempHandler(ErrorStatus.FAIL_DELETE);
    ...
    public String saveFile(MultipartFile file) {
        ...
        try {
            ...
            throw new TempHandler(ErrorStatus.FAIL_UPLOAD);
        } catch (SdkClientException e) {
    ...
    private String validateFileExtension(String originalFilename) {
        ...
        if (!allowedExtensions.contains(fileExtension)) {
            //throw new TempHandler("file", ErrorStatus.NOT_IMAGE_EXTENSION);
            throw new TempHandler(ErrorStatus.NOT_IMAGE_EXTENSION);
        }
